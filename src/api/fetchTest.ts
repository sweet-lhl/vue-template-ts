import http, {AxiosInstance} from '../plugins/http';
...
class FetchTest extends Api{
    ...
    fetchMockPing(): Promise<any>{
        return this.api.get('/ping' /* path: /api/test */, {params: {t: Date.now().valueOf()}}).then((r: any) => {
            console.log(`获取到mock-ping数据：${JSON.stringify(r)}`);
            return r;
        })
    }
}
